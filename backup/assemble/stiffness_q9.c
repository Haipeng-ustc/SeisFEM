...
void stiffness_q9(int node_num, int element_num, int element_order,  int *element_node, double **node_xy, double **stiffness)

 /* stiffness_q9 computes the stiffness matrix, using 9-node rectangular, 36 ponits quadrature rule.
...
*/
{
	...
	x2 = node_xy[0][p2]; 	y2 = node_xy[1][p2];
        ...
        if ( area == 0.0 ) 
        {
            printf("STIFFNESS_q9 - Fatal error!\n");
