...
void mass_q9(int node_num, int element_num, int element_order,  int *element_node, double **node_xy, double **mass)

 /* mass_q9 computes the mass matrix, using 9-node rectangular, 36 ponits quadrature rule.
		...
		for(jq = 0; jq < element_order; jq++)
                {
                  ...
                  mass[ip][jp] = mass[ip][jp] + area * weight[quad] * phi[iq] * phi[jq];
