 printf ( "\n Write some arrays into files. \n");
  if (( fp_element_node = fopen("element_node.txt","w")) == NULL) printf("\n File: element_node.txt cannot open\n");
  else
  ...
  if (( fp_node_xy = fopen("node_xy.txt","w")) == NULL) printf("\n File: node_xy.txt cannot open\n");
  else  
...
*/
 if (( fp_mass = fopen("mass_matrix_lump.txt","w")) == NULL) printf("\n File: mass_matrix_lump.txt cannot open\n");
  else  
  {	for( i = 0; i < node_num; i++) 
        {
